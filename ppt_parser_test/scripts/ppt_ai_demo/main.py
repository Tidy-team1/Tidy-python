...
for slide_idx, slide_instr in enumerate(instructions.get("slides", [])):
    ...
    for shape_instr in slide_instr.get("shapes", []):
        ...
        # ===== 이미지 수정 =====
            ...
            # 투명도 적용
            if "transparency" in shape_instr:
                # 기존 이미지 파일 경로 가져오기
                img_part = shape.image
                img_bytes = img_part.blob
                img_stream = io.BytesIO(img_bytes)
                # Pillow로 투명도 적용
                transparent_img = apply_transparency_to_image(img_stream, shape_instr["transparency"])
                # 기존 이미지를 대체
        ...
        # ===== 도형 수정 =====
            ...
            # 투명도 적용
            if "transparency" in shape_instr:
                set_fill_transparency(shape, shape_instr["transparency"])
